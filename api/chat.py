...
def run_with_tools(client, messages):
    ...
    # (which only actually runs against the local server, never this
    # Vercel function). Any image URLs found are collected separately -
    # they're not part of the spoken reply text.
    images = []
    for _ in range(3):
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            client = Anthropic()
            resp, images = run_with_tools(client, messages)
